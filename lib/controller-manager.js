...
var DbbCtrlMgr = Dbb.DbbObject.extend({
    ...
    getCtrl: function getCtrl(name) {
        return this._map && this._map[name];
    },

    addCtrl: function addCtrl(name, ctrl, options) {
        options = _.extend({}, this.options, options || {});

        if (options.autoRelease === undefined) options.autoRelease = true;
        if (!this._map) this._map = {};
        if (this._map[name]) return this; // console.error('已存在名为' +name+ '的控制器，请先使用removeCtrl再addCtrl');

        this._map[name] = {name: name, ctrlClass: ctrl, instance: null, options: options};
        return this;
    },

    removeCtrl: function removeCtrl(name) {
    ...
    load: function load(name, options, callback) {
        var self, ctrl;
        ...
        ctrl.instance.triggerEventMethod('controllerWillLoad', ctrl);
        ...
        if (this._current) {
            this.unload(this._current, options, function() {
                ...
                ctrl.instance.triggerEventMethod('controllerDidLoad', ctrl);
    ...
    unload: function unload(ctrl, options, callback) {
        var self = this;
        ...
        ctrl.instance.triggerEventMethod('controllerWillUnload');
        ...
        setTimeout(function() {
            ...
            ctrl.instance.triggerEventMethod('controllerDidUnload', ctrl);
