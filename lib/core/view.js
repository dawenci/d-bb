...
const DbbView = Backbone.View.extend({
    constructor: function DbbView(options) {
        ...
        // 如果是，触发相关生命周期钩子
        let el = _.result(options, 'el')
        let isMounted = el && isElMounted(el)
        if (isMounted) {
            let { supportLifeCycle, shouldPropagateViewWillMount } = this.$getOption(options, [
                'supportLifeCycle',
                'shouldPropagateViewWillMount'
            ])
        ...
        if (isMounted) {
            let { supportLifeCycle, shouldPropagateViewDidMount } = this.$getOption(options, [
                'supportLifeCycle',
                'shouldPropagateViewDidMount'
            ])
    ...
    $bindingRecords() {
        return this.__bindingRecords__ || []
    },
     ...
     */
    $dataForView(model) {
        return _.result(model, '$toDataForView', 
            _.result(model, 'toJSON', Object(model))
        )
    },
    ...
    $setTemplate(template) {
        delete this.__templateFunctionCache__
...
const extend = DbbView.extend = function(protoProps, staticProps) {
    ...
    if (protoProps) {
        ...
        if (protoProps.shouldMergeInitialize) {
            ...
            let _parentInit = Parent.prototype.initialize
            if (_init || _parentInit) {
                protoProps.initialize = function(options) {
                    if (_.isFunction(_parentInit)) _parentInit.call(this, options)
                    if (_.isFunction(_init)) _init.call(this, options)
                }
            }
        }
...
const dbbViewProto = DbbView.prototype
dbbViewProto.$isDestroyed = dbbViewProto.$isDealloc
dbbViewProto.$destroy = dbbViewProto.$dealloc
